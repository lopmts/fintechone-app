...
import { createCategorySchema, updateCategorySchema, listCategoriesSchema } from "../schemas/categories";
...
export async function categoryRoutes(app) {
    ...
    app.get("/", auth, async (request, reply) => {
        ...
        const [categories, total] = await Promise.all([
            prisma.category.findMany({ orderBy: { name: "asc" }, skip: (page - 1) * limit, take: limit }),
            prisma.category.count(),
        ]);
        return reply.send({ categories, pagination: { total, page, limit, pages: Math.ceil(total / limit) } });
    });
    // POST /categories
    app.post("/", auth, async (request, reply) => {
        ...
        try {
            ...
            if (error?.code === "P2002")
                return reply.status(409).send({ error: "Categoria com essa key já existe" });
            return reply.status(500).send({ error: "Erro ao criar categoria", detail: String(error) });
        }
    });
    ...
    app.put("/:id", auth, async (request, reply) => {
        ...
        if (exists) {
            ...
            if (updatePayload.color === undefined)
                delete updatePayload.color;
            const category = await prisma.category.update({ where: { id }, data: updatePayload });
    ...
    app.patch("/:id", auth, async (request, reply) => {
        ...
        try {
            ...
            if (error?.code === "P2002")
                return reply.status(409).send({ error: "Categoria com essa key já existe" });
            return reply.status(500).send({ error: "Erro ao atualizar categoria", detail: String(error) });
        }
    });
    ...
    app.delete("/:id?", auth, async (request, reply) => {
        ...
        const referenced = await prisma.transaction.count({ where: { categoryId: { in: targets } } });
        if (referenced > 0)
            return reply.status(400).send({ error: "Uma ou mais categorias possuem transações e não podem ser removidas" });
