...
export async function summaryRoutes(app: FastifyInstance) {
  ...
  app.get("/", auth, async (request, reply) => {
    ...
    const totalRealBalance = accountsWithBalance.reduce(
      (s, a) => s + a.realBalance,
      0,
    );
    const totalSalary = accounts.reduce(
      (s, a) => s + (a.salary?.toNumber() ?? 0),
      0,
    );
    const totalMonthExpenses = (
      monthExpenses._sum.amount ?? new Decimal(0)
    ).toNumber();
    const totalMonthIncome = (
      monthIncome._sum.amount ?? new Decimal(0)
    ).toNumber();
    const totalWeekExpenses = (
      weekExpenses._sum.amount ?? new Decimal(0)
    ).toNumber();
    const totalWeekIncome = (
      weekIncome._sum.amount ?? new Decimal(0)
    ).toNumber();

    const projectedBalance =
      totalRealBalance + totalSalary - totalMonthExpenses;
    ...
    const installmentsDueThisMonth = financings.flatMap((f) =>
      f.installmentsData
        .filter(
          (i) =>
            !i.isPaid &&
    ...
    const totalDueThisMonth = Number(
      installmentsDueThisMonth.reduce((s, i) => s + i.amount, 0).toFixed(2),
    );
    const totalOverdue = Number(
      overdueInstallments.reduce((s, i) => s + i.amount, 0).toFixed(2),
    );
