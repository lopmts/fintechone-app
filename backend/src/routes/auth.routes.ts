...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      return reply.status(201).send({
        user,
        message:
          "Conta criada. Verifique seu e-mail e use POST /auth/verify com o código de 6 dígitos.",
  ...
  app.post("/login", async (request, reply) => {
    ...
    if (password) {
      if (!user.password) {
        return reply.status(400).send({
          error:
            "Esta conta não possui senha. Use o fluxo de código por e-mail.",
        });
      }
    ...
    return reply.send({
      message:
        "Código enviado para seu e-mail. Use POST /auth/verify para autenticar.",
      requiresVerification: true,
    });
  });
  ...
  app.post("/password-reset", async (request, reply) => {
    ...
    if (user) await issueAndSendCode(user.id, email, user.name);
