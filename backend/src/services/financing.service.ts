...
export interface Installment {
  ...
  paidAmount?: number;
  paidAt?: Date;
  // ── Penalidade por atraso (presente apenas quando há atraso) ──
  daysLate?: number;
...
export interface FinancingWithInstallments {
  ...
  title?: string | null;
  isActive: boolean;
  amount: number; // valor financiado (informativo)
  installmentAmount: number; // valor da parcela, informado pelo usuário
  interestRate?: number | null; // % ao mês — opcional, apenas informativo
  installments: number;
  startDate: Date;
...
export interface CreateFinancingInput {
  ...
  title?: string | null;
  amount: number;
  installmentAmount: number; // valor da parcela — definido pelo usuário, não calculado
  interestRate?: number | null; // opcional, apenas informativo
  installments: number;
  startDate: Date;
  lateFeeRate?: number | null;
  lateInterestRate?: number | null;
...
export type UpdateFinancingInput = Partial<
  Omit<CreateFinancingInput, "userId">
>;
 ...
 */
function getDueDate(startDate: Date, installmentNumber: number): Date {
 ...
 */
function daysBetween(dueDate: Date, referenceDate: Date): number {
...
function calcPenalty(
  ...
  dueDate: Date,
  referenceDate: Date,
  lateFeeRate: number | null | undefined,
  lateInterestRate: number | null | undefined,
): {
  daysLate: number;
  lateFeeApplied: number;
  lateInterestApplied: number;
  ...
  const lateInterestApplied = lateInterestRate
    ? Number(
        ((installmentAmount * lateInterestRate * daysLate) / 100).toFixed(2),
      )
    : 0;

  const totalWithPenalty = Number(
    (installmentAmount + lateFeeApplied + lateInterestApplied).toFixed(2),
  );
...
function buildFinancingWithInstallments(f: {
  ...
  lateInterestRate?: Prisma.Decimal | null;
  installmentsPaid: {
    installmentNumber: number;
    amount: Prisma.Decimal;
    paidAt: Date;
  }[];
}): FinancingWithInstallments {
  const now = new Date();
  ...
  const lateInterestRate =
    f.lateInterestRate != null ? Number(f.lateInterestRate) : null;

  const paidByNumber = new Map(
    f.installmentsPaid.map((p) => [p.installmentNumber, p]),
  );

  const installmentsData: Installment[] = Array.from(
    { length: n },
    (_, idx) => {
      const number = idx + 1;
  ...
  const totalPenaltiesAccrued = Number(
    installmentsData
      .filter((i) => !i.isPaid && (i.daysLate ?? 0) > 0)
      .reduce(
        (s, i) => s + (i.lateFeeApplied ?? 0) + (i.lateInterestApplied ?? 0),
        0,
      )
      .toFixed(2),
  );
...
export async function getFinancings(
  ...
  const filtered = enriched.filter((f) => {
    ...
    if (filter === "paid") return isFullyPaid;
    if (filter === "unpaid") return !isFullyPaid;
  ...
  const summary = {
    ...
    totalRemaining: Number(
      filtered.reduce((s, f) => s + f.remainingAmount, 0).toFixed(2),
    ),
    totalPenaltiesAccrued: Number(
      filtered.reduce((s, f) => s + f.totalPenaltiesAccrued, 0).toFixed(2),
    ),
  };
...
export async function getFinancingById(
  userId: string,
  financingId: string,
): Promise<FinancingWithInstallments | null> {
...
export async function updateFinancing(
  ...
  if (!existing) throw new Error("Financing not found");
  ...
  if (
    ...
    throw new Error(
      `Cannot set installments below ${existing.installmentsPaid.length} (already paid)`,
    );
  }

  const data: Prisma.FinancingUpdateInput = {};
...
export async function markInstallmentsPaid(
  userId: string,
  input: MarkInstallmentsPaidInput,
): Promise<FinancingWithInstallments> {
  ...
  const alreadyPaid = new Set(
    financing.installmentsPaid.map((i) => i.installmentNumber),
  );
  ...
  return refreshed as FinancingWithInstallments;
}
