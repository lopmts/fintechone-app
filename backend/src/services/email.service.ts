...
async function sendViaSMTP({
  ...
  const nodemailer = await import("nodemailer").catch(() => {
    throw new Error(
      "Instale nodemailer: npm install nodemailer @types/nodemailer",
    );
  });
...
export async function sendDeleteWarningEmail({
  to,
  userName,
  expiresAt,
}: SendDeleteWarningOptions) {
  const name = userName ?? "usuário";
...
export async function sendReactivateCodeEmail({
  to,
  userName,
  code,
}: SendReactivateCodeOptions) {
  const name = userName ?? "usuário";
